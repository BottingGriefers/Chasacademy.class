...
while killerapp:
    ...
    if menu == "1": 
        #varför en dictionary? jo för att uppgiften sa du ska visa var i listan det Fanns namn och older
        students = { 
            "Name": input("What are you called?: "),
            "Age": int(input("How many years have you lived?: "))}
        
        # vi andvänder oss av append för att lägga till nya studenter inom listan med deras nya dictionary
        studentslist.append(students) 

        print("Thank you for adding yourself to the database ")
    #här skriver den ut eleverna vilka som finns i en lista och dictionary 
    elif menu == "2":

        print(f"Here are the names of our students {studentslist} ")

    #här hade jag lite roligt, när man lägger till studenter så efter åt kan man eliminera dem vilket görs att det försvinner från listan
    elif menu == "3":
        find_student = input("Enter whom you want to eliminate!!!: ")
         # namnet man uppger, inom find_student som är i studentslist försvinner hen 
        for students in studentslist:
            if students["Name"].lower == find_student.lower():
