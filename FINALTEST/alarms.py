...
class AlarmManager: 
    ...
    def critical_alert(self, message):
        ...
        root.configure(bg="#32a0a8")  # background color
    ...
    #Display the options for configuring alarms.
        ...
        "2. Memory usage\n"
        "3. Disk usage\n"
        "4. Back to main menu", "[bold][#fc035a]=== Create an Alarm ===[/#fc035a][/bold]", "[#fc035a][bold]↓↓↓↓↓↓[/bold][/#fc035a]"
        )
        return 
    ...
    def delete_alarm(self, hardware_type):
        ...
        console.print(Align(f"[#03fc62][bold]All alarms for {hardware_type} have been removed.[/#03fc62][/bold]",align="center"))

    def delete_alarm_menu(self):
        delete_input = console.input(
            Align("[#fc035a][bold]Which alarm would you like to delete[/#fc035a]\n[#ffffff]| 1. CPU | 2. Memory | 3. Disk |[#ffffff][/bold]",
                align="center"))
        ...
        if hardware_type:
            ...
            console.print("Invalid choice, no alarms deleted.")

    def add_alarm(self, hardware_type, level):
        """Add an alarm threshold for CPU, memory, or disk."""
        if hardware_type not in self.alarms:
            console.input(Align(f"[#fc035a][bold]Unknown type: {hardware_type}[/#fc035a]\n[#ffffff]| 1. CPU | 2. Memory | 3. Disk |[#ffffff][/bold]",align="center"))
            return
        ...
        if not (1 <= level <= 100):
            console.print(Align("[#fc035a][bold]Level must be between 1 and 100%[/#fc035a][/bold", align="center"))
            return
        
        #this appends it to the library
        self.alarms[hardware_type].append(level)
        console.print(Align(f"[#03fc62][bold] Added alarm for[/#03fc62][/bold] [bold][#0349fc]{hardware_type} [/bold][/#0349fc][#03fc62][bold]at[/#03fc62][/bold] [bold][#0349fc]{level}[/bold][#/0349fc][#03fc62][bold]%[/#03fc62][/bold]\n",align="center"))
    ...
    def show_alarms(self):
        ...
        for hardware, levels in self.alarms.items():
            if levels:
                for lvl in levels:
                    console.print(Align(f"[bold][#0349fc]{hardware.capitalize()} alarm: [/bold][/#0349fc][#fc035a][bold]{lvl}%[/#fc035a][/bold]",align="center"))
            else:
                console.print(Align(f"[bold][#0349fc]{hardware.capitalize()} alarm: [/bold][/#0349fc][#fc035a][bold]\-/[/#fc035a][/bold]",align="center"))
        # show all alarms
    ...
    def menu_input_choice(self):
        while True:
            ...
            choice = console.input(Align("[#fc035a][bold]what is your choice[/#fc035a][/bold]", align="center")).strip().lower()
            if choice == "1":
                hardware = "cpu"
            elif choice == "2":
                hardware = "memory"
                ...
                console.print(Align(f"[#ffffff][bold]Invalid choice. Please restart the program and choose 1-4.\n[/bold][/#ffffff]", align="center"))
                continue
            while True:  
                if hardware:  # only continue if a valid hardware type was chosen
                    ...
                    if level_input.isdigit() and 1 <= int(level_input) <= 100:
                        ...
                        console.print(Align(f"[#ffffff][bold]That is not a valid number between 1 and 100. No alarm created.[/bold][/#ffffff]", align="center"))
                        continue  
                return
    ...
    def check_alarms(self, stats):
        ...
        if alerts:
            console.print(Align("[#fc035a][bold]===ALARM TRIGGERED!===[/bold][/#fc035a]\n", align="center"))
            for msg in alerts:
                console.print(Align(f"[#fc035a]{msg}[/#fc035a]", align="center"))
            alarm_manager.custom_alarm_sound()
            alarm_manager.critical_alert("WARNING! CRITICAL ALERT! CHECK SYSTEM USAGE!\n AND LOVE YOUR COMPUTER! :)")
